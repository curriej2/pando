...
#
#  Usage: bash src/47_submit_B1000.sh [--dry]
# =============================================================================
set -euo pipefail
...
sub() {  # sub <name> <time> <extra sbatch args...> -- <command>; echoes the job id
  ...
  sbatch -A lesliec -p lesliec,cpu -c 1 --mem 8G -t "$tm" -J "$name" \
...
sbatch -A lesliec -p lesliec,cpu -c 1 --mem 16G -t 6:00:00 -J perm_collect \
